...
/**
 * Represents a composite label that combines a {@link LabelCategory} with a {@link Label}.
 ...
 *
 * The class ensures immutability by using final fields for its components: {@code category}
 * and {@code label}. Subclasses provide specific categorizations by invoking the constructor
 * with a predefined category and a {@link Label}.
 *
 * Equality and hash code calculations are based on the category, label type, and label value.
 * The string representation summarizes the composite label with its category, type, and value.
 */
