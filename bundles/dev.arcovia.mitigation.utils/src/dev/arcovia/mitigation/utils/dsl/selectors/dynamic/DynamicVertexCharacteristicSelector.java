...
import dev.arcovia.mitigation.utils.dsl.selectors.constant.ConstantVertexCharacteristicListSelector;
...
/**
 * Wraps a {@link VertexCharacteristicsSelector} to provide dynamic vertex characteristic selection. Converts variable
 * values into constant vertex characteristic literals for a {@link BranchNode} and generates {@link Label} objects for
 * dynamic vertex characteristics based on provided variables.
 */
public class DynamicVertexCharacteristicSelector implements DynamicDataSelector {
    ...
    /**
     * Adds literals to the root node based on the dynamic vertex characteristic values. Converts variable values into
     * constant vertex characteristic selectors and delegates literal addition. Throws an exception if no variables are
     * found.
    ...
    public void addLiterals(BranchNode root, Map<String, List<String>> variables, boolean inverted) {
        List<String> vars = variables.get(selector.getVertexCharacteristics()
                .characteristicValue()
                .name());
        ...
        List<CharacteristicsSelectorData> data = vars.stream()
                .map(it -> new CharacteristicsSelectorData(selector.getVertexCharacteristics()
                        .characteristicType(), ConstraintVariableReference.ofConstant(List.of(it))))
                .toList();
        new ConstantVertexCharacteristicListSelector(new VertexCharacteristicsListSelector(null, data, inverted)).addLiterals(root);
    }
