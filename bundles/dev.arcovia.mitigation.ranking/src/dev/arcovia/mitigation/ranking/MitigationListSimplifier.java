...
/*
 * Class for simplification of a list of mitigation models. Marks which uncertainties got removed "wrongly".
 */
public class MitigationListSimplifier {
        ...
        for (int i = 0; i < models.size(); i++) {
            for (int j = 0; j < models.size(); j++) {
                ...
                if (differingIndex != -1) {
                    ...
                    var firstScenarios = models.get(i)
                            .get(differingIndex)
                            .split("&");
                    var secondScenarios = models.get(j)
                            .get(differingIndex)
                            .split("&");
                    ...
                    if (newScenarioSum == scenarioAmounts.get(differingIndex)) {
                        ...
                        newChosenScenarios.set(differingIndex, models.get(i)
                                .get(differingIndex) + "&"
                                + models.get(j)
                                        .get(differingIndex));
                    }
    ...
    private static int differByOneElement(List<String> mitigationModel1, List<String> mitigationModel2) {
        ...
        for (int i = 0; i < mitigationModel1.size(); i++) {
            if (mitigationModel1.get(i)
                    .equals("U")
                    || mitigationModel2.get(i)
                            .equals("U")) {
                continue;
            }
            if (!mitigationModel1.get(i)
                    .equals(mitigationModel2.get(i))) {
